...
content = content.replace('animate-[fadeInUp_0.4s_ease-out]', '${activeTab === "home" ? "" : "animate-[fadeInUp_0.4s_ease-out]"}')
...
content = content.replace('className="min-h-screen overflow-x-hidden bg-[#030712]/30 backdrop-blur-md text-slate-50', 'className="min-h-screen overflow-x-hidden text-slate-50')
codecs.open(home_path, 'w', 'utf-8').write(content)
print('Fixed stacking context issues for video background')
