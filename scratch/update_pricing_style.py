...
start_str = '<div className="grid md:grid-cols-2 p-6 md:p-10 gap-8 bg-white">'

s = content.find(start_str)
e = content.find('<Header ', s)

if s != -1 and e != -1:
    old_grid = content[s:e]
    # We must preserve the closing tags before Header
    # old_grid should end with something like:
    # </div>
    #       </div>
    #     </div>
    #   )}
    ...
    # Let's extract everything from s to e-1
    old_grid = content[s:e]
    
    # We need to construct the new grid to end exactly where the old grid ended.
    # We'll just replace the inner part. The inner part is from start_str to the end of the two column divs.
    # It's easier if we just write the whole block and append the closing tags.
...
"""
    content = content[:s] + new_grid + content[e-8:] 
    # e-8 to preserve the spaces and <Header 
    # Let's just do content = content[:s] + new_grid + content[e:] but make sure new_grid ends with exactly what's needed.
    # Actually wait. Let's just replace the substring.
    
    with codecs.open(filepath, 'w', 'utf-8') as f:
        f.write(content[:s] + new_grid + content[e:])
    print('Updated successfully!')
else:
    print('Target not found', s, e)
