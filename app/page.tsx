...
export default function Home() {
  ...
  return (
    <div className={`min-h-screen flex flex-col ${activeTab === 'home' ? 'bg-[#030712]' : 'bg-[#F3F4F6] dark:bg-[#09090b]'} text-gray-800 font-sans selection:bg-blue-200 relative z-0`}>
      {/* GLOBAL BACKGROUND FOR FINES TAB */}
      {(activeTab === 'home' || activeTab === 'fines' || activeTab === 'admin' || activeTab === 'evaluate' || activeTab === 'analytics') && (
        <div className="fixed inset-0 pointer-events-none z-[-1] overflow-hidden">
          <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] invert dark:invert-0 opacity-[0.25]"></div>
          ...
          <div className="absolute top-1/3 -left-20 w-[35rem] h-[35rem] bg-blue-300/40 dark:bg-blue-500/20 rounded-full blur-[120px] animate-[pulse_7s_ease-in-out_infinite_alternate]"></div>
        </div>
      )}
      ...
      {user && user.tier === 'vip' && (
        <button 
          ...
          className="fixed bottom-6 left-6 z-[999] bg-red-600 hover:bg-red-700 text-slate-900 dark:text-white px-4 py-2 rounded-full font-bold shadow-xl shadow-red-600/30 text-sm flex items-center gap-2 transition-all active:scale-95"
        >
          🛠️ Xóa VIP (Test lại)
        </button>
      )}
      ...
      {showPricingModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bgtext-gray-900/80 backdrop-blur-sm animate-[fadeIn_0.3s_ease-out]">
          <div className="bg-white rounded-[2rem] shadow-2xl max-w-4xl w-full overflow-hidden border border-gray-100 relative">
            <button onClick={() => !isProcessingPayment && setShowPricingModal(false)} className="absolute top-6 right-6 texttext-gray-600 dark:text-gray-400 hover:texttext-gray-900 hover:bg-gray-100 p-2 rounded-full transition-all z-20">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
            </button>
            <div className="p-10 text-center border-b border-gray-100 bg-gray-50/50">
              <h2 className="text-3xl md:text-4xl font-black texttext-gray-900 mb-3 tracking-tight">Nâng Tầm Trải Nghiệm Định Giá</h2>
              <p className="text-gray-700 dark:texttext-gray-500">Chọn gói dịch vụ phù hợp với nhu cầu của bạn để tối ưu hóa lợi nhuận.</p>
            </div>
            <div className="grid md:grid-cols-2 p-6 md:p-10 gap-8 bg-white">
              <div className="p-8 rounded-3xl border-2 border-gray-100 bg-gray-50 flex flex-col">
                ...
                <p className="text-4xl font-black texttext-gray-900 mb-6">Miễn phí</p>
                <ul className="space-y-4 mb-8 flex-1 text-sm font-medium texttext-gray-600">
                  <li className="flex items-center gap-3"><span className="text-emerald-600 dark:text-emerald-500 text-lg">✓</span> Phân tích AI 30+ trường dữ liệu</li>
                  <li className="flex items-center gap-3"><span className="text-emerald-600 dark:text-emerald-500 text-lg">✓</span> Cấp mộc chứng nhận Blockchain</li>
                  <li className="flex items-center gap-3 texttext-gray-900"><span className="text-amber-500 text-lg">⚡</span> Giới hạn 3 lượt / ngày</li>
                  <li className="flex items-center gap-3 opacity-50"><span className="text-red-700 dark:text-red-400 text-lg">✗</span> Phí Gas Web3: 0.001 ETH / lần</li>
                </ul>
                <button className="w-full py-4 rounded-xl font-bold text-gray-700 dark:texttext-gray-500 bg-gray-200 cursor-not-allowed">Đang sử dụng</button>
              </div>
              <div className="p-8 rounded-3xl border-2 border-yellow-300 bg-gradient-to-b from-yellow-50 to-white flex flex-col relative shadow-xl shadow-yellow-500/10">
                <div className="absolute top-0 right-0 transform translate-x-2 -translate-y-4">
                  <span className="bg-red-500 text-slate-900 dark:text-white text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest shadow-md">Được khuyên dùng</span>
                </div>
                <h3 className="text-xl font-black text-yellow-700 mb-2 flex items-center gap-2">👑 Tài khoản VIP (Dealer)</h3>
                <div className="mb-6 flex items-baseline gap-2">
                  <p className="text-4xl font-black texttext-gray-900">0.05<span className="text-xl text-blue-600 ml-1">ETH</span></p>
                  <p className="texttext-gray-600 dark:text-gray-400 font-bold">/ tháng</p>
                </div>
                <ul className="space-y-4 mb-8 flex-1 text-sm font-bold text-gray-800">
                  <li className="flex items-center gap-3 text-emerald-600"><span className="text-emerald-600 dark:text-emerald-500 text-lg">♾️</span> <strong>Không giới hạn</strong> lượt định giá / ngày</li>
                  <li className="flex items-center gap-3 text-blue-600"><span className="text-blue-600 dark:text-blue-500 text-lg">🚀</span> Miễn phí 100% phí Gas Web3</li>
                  <li className="flex items-center gap-3"><span className="text-emerald-600 dark:text-emerald-500 text-lg">✓</span> Mở khóa Dự báo Rớt giá 1 năm</li>
                </ul>
                <button 
                  onClick={processVIPUpgrade} 
                  disabled={isProcessingPayment}
                  className={`w-full py-4 rounded-xl font-black text-slate-900 dark:text-white transition-all duration-300 shadow-lg active:scale-95 flex items-center justify-center gap-2 ${isProcessingPayment ? "bg-yellow-400 cursor-wait" : "bg-gradient-to-r from-yellow-400 to-amber-500 hover:from-yellow-300 hover:to-amber-400 hover:shadow-yellow-500/40"}`}
                >
      ...
      <main key={activeTab} className={`flex-grow mx-auto w-full animate-[fadeInUp_0.4s_ease-out] ${(activeTab === 'home' || activeTab === 'analytics' || activeTab === 'fines' || activeTab === 'marketplace') ? 'max-w-full px-0 pt-0 pb-0' : 'px-6 pt-24 pb-20'} ${(activeTab === 'evaluate' || activeTab === 'search' || activeTab === 'admin') ? 'max-w-full px-2 md:px-6' : (activeTab !== 'home' && activeTab !== 'analytics' && activeTab !== 'fines' && activeTab !== 'marketplace' ? 'max-w-6xl' : '')}`}>
        ...
        {activeTab === "marketplace" && <MarketplaceTab user={user} />}
        {activeTab === "penalty" && <PenaltyTab/>}
        {activeTab === "admin" && <AdminTab />} 
      </main>

      <footer className={`no-print border-t mt-auto w-[100vw] relative left-1/2 -translate-x-1/2 border-black/10 dark:border-white/10 ${activeTab === "home" ? "bg-[#030712] text-gray-300" : "bg-transparent text-slate-800 dark:text-gray-300"}`}>
        <div className="max-w-[1400px] mx-auto px-6 py-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          {/* Cột 1 */}
          <div className="flex flex-col gap-4">
            <div className={`flex items-center gap-2 font-black text-2xl ${activeTab === "home" ? "text-white" : "text-slate-900 dark:text-white"}`}>
