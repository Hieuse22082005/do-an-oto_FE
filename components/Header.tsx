...
export default function Header({ user, activeTab, onTabChange, onLoginClick, onLogoutClick, onUpgradeVIP }: HeaderProps) {
  // Logic xử lý Theme (Trắng - Đen - Huyền ảo)
  ...
  return (
    <header className="no-print bg-white/90 backdrop-blur-md shadow-sm sticky top-0 z-50 transition-all duration-300 border-b border-gray-100">
      <div className="max-w-6xl mx-auto px-6 py-4 flex justify-between items-center">
        ...
        {/* LOGO NHẬN DIỆN THƯƠNG HIỆU VỚI ICON XE (CAR) TƯƠNG LAI */}
        <div className="flex items-center gap-3 cursor-pointer group" onClick={() => onTabChange("home")}>
          ...
          <span className="font-extrabold text-2xl tracking-tighter text-gray-900 flex items-baseline">
            địnhgiá
        ...
        <div className="flex items-center gap-8 text-sm font-bold text-gray-600">
          <span onClick={() => onTabChange("home")} className={`relative cursor-pointer py-2 group transition-colors duration-300 hover:text-blue-600 ${activeTab === "home" ? "text-blue-600" : ""}`}>
            Trang chủ
          ...
          <span onClick={() => onTabChange("evaluate")} className={`relative cursor-pointer py-2 group transition-colors duration-300 hover:text-blue-600 ${activeTab === "evaluate" ? "text-blue-600" : ""}`}>
            Định giá AI
          ...
          <span onClick={() => onTabChange("search")} className={`relative cursor-pointer py-2 group transition-colors duration-300 hover:text-blue-600 ${activeTab === "search" ? "text-blue-600" : ""}`}>
            Tra cứu TxHash
            ...
            {user ? (
              <div className="flex items-center gap-2">
                <div className="bg-gray-50 border border-gray-100 flex items-center gap-2 px-3 py-1.5 rounded-xl cursor-default transition-all duration-300">
                  <span className="text-gray-800 font-medium text-sm">👋 {user.user_metadata?.display_name || user.email.split('@')[0]}</span>
                  
                  {user.tier === 'vip' ? (
                    <span className="bg-gradient-to-r from-yellow-400 to-amber-500 text-white text-[10px] font-black px-2 py-0.5 rounded uppercase tracking-widest shadow-sm shadow-yellow-500/30 border border-yellow-200">
                      👑 VIP
                    </span>
                  ) : (
                    <button onClick={onUpgradeVIP} className="bg-blue-100 hover:bg-blue-200 text-blue-700 text-[10px] font-bold px-2 py-0.5 rounded transition-colors uppercase">
                      Nâng cấp VIP
                    </button>
                  )}
                </div>

                <button onClick={onLogoutClick} className="text-gray-400 hover:text-red-500 hover:bg-red-50 p-2 rounded-lg transition-all duration-300" title="Đăng xuất">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"></path></svg>
                </button>
              </div>
            ) : (
              <button onClick={onLoginClick} className="bg-blue-600 text-white px-5 py-2.5 rounded-xl hover:bg-blue-700 active:scale-95 transition-all duration-300 font-bold">Đăng nhập</button>
            )}

            {/* NÚT PHÉP THUẬT ĐỔI MÀU GIAO DIỆN */}
            <button 
              onClick={cycleTheme} 
              className="w-10 h-10 ml-2 rounded-full bg-gray-50 border border-gray-200 hover:bg-blue-50 hover:border-blue-200 hover:scale-110 active:scale-95 flex items-center justify-center text-xl transition-all duration-300 shadow-sm"
