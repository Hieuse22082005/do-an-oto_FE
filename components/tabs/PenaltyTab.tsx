...
export default function PenaltyTab() {
  ...
  return (
    // Đã đổi bg-[#F8F9FA] thành bg-[#F3F4F6] để nhìn xuyên thấu chữ từ page.tsx
    <div className="w-[100vw] relative left-1/2 -translate-x-1/2 bg-[#F3F4F6] pb-20 font-sans transition-colors duration-300 overflow-hidden -mt-12">
      ...
      {/* Header Banner (Đổi bg-gradient thành bg-[#F3F4F6]) */}
      <div className="relative z-10 text-center py-12 border-b border-transparent  transition-colors duration-300 bg-transparent">
        <div className="inline-flex items-center gap-2 bg-red-50  text-red-600  px-4 py-1.5 rounded-full text-sm font-semibold mb-4 border border-red-100  shadow-sm animate-pulse">
        ...
        <h1 className="text-4xl font-black text-gray-900  mb-4 tracking-tight transition-colors duration-300">
          Cẩm Nang Luật <span className="text-[#E53935] ">Ô Tô</span>
        </h1>
        <p className="text-gray-500  max-w-xl mx-auto transition-colors duration-300">
      ...
      <div className="max-w-[1600px] mx-auto px-4 lg:px-12 grid grid-cols-1 lg:grid-cols-12 gap-10 pt-8">
        ...
        <div className="lg:col-span-3">
          <div className="bg-white  border border-gray-200  rounded-2xl overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)] sticky top-24 transition-colors duration-300">
            {CATEGORIES.map((cat) => {
              ...
              return (
                <button 
                  ...
                  className={`w-full group relative text-left px-5 py-4 border-b border-gray-100  flex items-center gap-3 text-[14px] font-semibold transition-all duration-300 overflow-hidden
                    ${isActive 
                      ? (isPopularTab ? 'text-orange-600  bg-orange-50/50 ' : 'text-[#E53935]  bg-red-50/50 ') 
                      : 'text-gray-600  hover:bg-gray-50 '}`}
                >
                  <span className={`absolute left-0 top-0 bottom-0 w-1 transition-all duration-300 ease-out ${isActive ? (isPopularTab ? 'bg-orange-500' : 'bg-[#E53935] ') : 'bg-[#F3F4F6] group-hover:bg-gray-300 '}`}></span>
                  ...
                  {isActive && (
                    <span className={`ml-auto text-white text-[10px] px-2 py-0.5 rounded-full shadow-sm animate-fade-in ${isPopularTab ? 'bg-orange-500' : 'bg-[#E53935] '}`}>
        ...
        <div className="lg:col-span-6">
          <div className="mb-6 text-gray-800  font-bold text-lg flex items-center gap-2 transition-colors duration-300">
            Hiển thị: <span className={activeCategory === 'popular' ? 'text-orange-600 ' : 'text-[#E53935] '}>{CATEGORIES.find(c => c.id === activeCategory)?.name}</span> 
            <span className="text-gray-400  font-normal text-sm">({filteredViolations.length} quy định)</span>
          </div>
          ...
          <div className="space-y-4">
            {filteredViolations.map((violation) => {
              ...
              return (
                <div key={violation.id} className={`bg-white  border rounded-2xl overflow-hidden transition-all duration-300 shadow-sm
                  ${isExpanded 
                    ? 'border-red-300  shadow-md shadow-red-100/50  ring-2 ring-red-50 ' 
                    : 'border-gray-200  hover:border-red-200  hover:shadow-md'}`}>
                  
                  <button onClick={() => setExpandedId(isExpanded ? null : violation.id)} className="w-full px-6 py-5 flex items-center justify-between bg-[#F3F4F6] focus:outline-none group">
                    <span className={`font-bold text-[15px] text-left pr-4 transition-colors duration-300 ${isExpanded ? 'text-[#E53935] ' : 'text-gray-800  group-hover:text-red-600 '}`}>
                      {violation.title}
                    </span>
                    <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${isExpanded ? 'bg-red-100  text-[#E53935]  rotate-180' : 'bg-gray-100  text-gray-500  group-hover:bg-red-50  group-hover:text-red-500 '}`}>
                  ...
                  <div className={`grid transition-[grid-template-rows] duration-300 ease-in-out ${isExpanded ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}>
                    <div className="overflow-hidden">
                      <div className="px-6 pb-6 pt-2">
                        <div className="bg-gray-50  p-5 rounded-xl border border-gray-100  relative transition-colors duration-300">
                          <h5 className="font-bold text-gray-700  text-sm mb-3 border-b border-gray-200  pb-2 inline-block">{violation.oldDecree}</h5>
                          <p className="text-gray-600  text-[15px] mb-2 flex gap-2"><span className="font-semibold w-28 shrink-0">Mức phạt tiền:</span> <span className="font-bold text-gray-900 ">{violation.oldFine}</span></p>
                          {violation.oldExtra && <p className="text-gray-600  text-[15px] flex gap-2"><span className="font-semibold w-28 shrink-0">Hình phạt phụ:</span> <span className="font-medium text-gray-800 ">{violation.oldExtra}</span></p>}
                        </div>
                        <div className="flex justify-center -my-3.5 relative z-10">
                          <div className="bg-white  p-1.5 rounded-full border border-gray-200  text-red-500  shadow-sm transition-colors duration-300">
                        ...
                        <div className="bg-[#FFF5F5]  p-5 rounded-xl border border-red-100  mt-2 transition-colors duration-300">
                          <h5 className="font-bold text-red-700  text-sm mb-3 border-b border-red-200/50  pb-2 inline-block">{violation.newDecree}</h5>
                          <p className="text-gray-700  text-[15px] mb-2 flex gap-2"><span className="font-semibold w-28 shrink-0">Mức phạt tiền:</span> <span className="font-black text-red-600  text-base">{violation.newFine}</span></p>
                          {violation.newExtra && <p className="text-gray-700  text-[15px] flex gap-2"><span className="font-semibold w-28 shrink-0">Quy định mới:</span> <span className="font-bold text-red-700  bg-red-100  px-2 py-0.5 rounded text-sm">{violation.newExtra}</span></p>}
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        <div className="lg:col-span-3 space-y-6 sticky top-24 h-fit">
          ...
          <div className="bg-gradient-to-br from-red-500 to-red-600   rounded-2xl p-5 text-white shadow-[0_8px_30px_rgb(229,57,53,0.3)]  transition-colors duration-300">
            ...
            <div className="bg-white/20  rounded-xl p-3 text-center font-black text-2xl tracking-widest backdrop-blur-sm border border-white/30 ">
              08 68 911 911
            </div>
          </div>

          <div className="bg-white  border border-gray-200  rounded-2xl p-5 shadow-sm transition-colors duration-300 group">
            <div className="flex items-center justify-between mb-3">
              <span className="text-[10px] font-black text-gray-400  tracking-wider">TÀI TRỢ CHÍNH</span>
              <span className="bg-indigo-100  text-indigo-600  text-[10px] font-bold px-2 py-0.5 rounded">Ad</span>
            </div>
            ...
            <h4 className="font-bold text-gray-800  text-[15px] leading-tight mb-1 group-hover:text-indigo-600  transition-colors">
              Camera Hành Trình 4K OTOCHECK
            </h4>
            <p className="text-xs text-gray-500  font-medium mb-4">Cảnh báo tốc độ, đọc biển báo giao thông thông minh. Độc quyền cho thành viên.</p>
            <button className="w-full bg-indigo-50  text-indigo-600  border border-indigo-100  font-bold text-sm py-2.5 rounded-xl hover:bg-indigo-600 hover:text-white   transition-all duration-300 active:scale-95">
              Tìm Hiểu Thêm
            </button>
          </div>
